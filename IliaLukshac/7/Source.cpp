...
int main()
{
	const char* fname = "D:\\File\\hotel.txt";
	char choice;
	char str[120];
	...
	system("cls");
	printf_s("\nWrite your request like \"Venice lux room for two 100\"\n");
	printf_s("Input: ");
	getchar();
	gets_s(str, 100);
	int x = 0;
	...
	printf_s("\nEnter the index of the hotel u'd like to book\n");
...
bool reserveRoom(struct Hotel* hotels, int hotelAmount, int index)
{
	...
	for (int i = 0; i < hotelAmount; i++)
	{
		if (index == i)
		{
			FILE* f = fopen("D:\\File\\hotel.txt", "a");
			if (!f)
			{
				printf_s("Error");
				return 0;
